...
// Get Dashboard Stats (Hybrid: Log for recent, Sums for totals)
    ...
    try {
        const result = await db.query(`
            SELECT 
                ...
                -- 2. Downloads
                -- "All Time": Sum existing counters from ANIME table (Preserves history)
                (SELECT COALESCE(SUM(download_count), 0) FROM animes) :: int as total_downloads,
                -- Intervals: Count specific rows from ANALYTICS table
                ...
                -- 3. Visits
                -- "All Time": Sum existing counters from ANIME table
                ...
                (SELECT COUNT(*) FROM analytics WHERE type = 'visit' AND created_at >= NOW() - INTERVAL '30 DAYS') :: int as visits_month
        `);
...
// Create or Update Anime Metadata
...
// Delete Anime (Cascades to analytics logs, episodes, clips)
...
router.post('/clips/bulk', async (req, res) => {
    ...
    try {
        ...
        if (animeRes.rows.length === 0) throw new Error(`Anime '${anime}' not found.`);
        ...
        if (epRes.rows.length === 0) {
            epRes = await client.query('INSERT INTO episodes (anime_id, number, title) VALUES ($1, $2, $3) RETURNING id', [animeId, episode, `Episode ${episode}`]);
        }
        ...
        for (const c of clips) {
            await client.query(
                ...
                [epId, c.video_url, c.preview_url || '', c.thumb_url || '', c.clip_name || 'Clip', c.start_time || 0, c.end_time || 0]
            );
        }
        ...
        for (const ep of episodes) {
            ...
            for (const clip of ep.clips) {
                await client.query(
                    ...
                    VALUES ($1, $2, $3, $4, $5, $6, $7)`,
                    [epId, 0, clip.duration, clip.video_url, clip.preview_url||'', clip.thumb_url||'', clip.original_clip_name]
                );
            }
        }
        await client.query('COMMIT');
        res.json({ status: 'success', message: 'Season imported' });
    } catch (err) {
        await client.query('ROLLBACK');
        res.status(500).json({ status: 'error', message: err.message });
    } finally { client.release(); }
